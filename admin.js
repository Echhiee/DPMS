...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderRegistrationsTable() {
    ...
    if (registrations.length === 0) {
      registrationsTableBody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px; color: #999;">No pending registrations</td></tr>';
      return;
    }
  ...
  function renderPatientsTable(query) {
    ...
    if (filtered.length === 0) {
      patientsTableBody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px; color: #999;">No patients found</td></tr>';
      return;
    }
    ...
    filtered.forEach((patient) => {
      ...
      const statusColor = patient.status === 'approved' ? 'green' : patient.status === 'rejected' ? 'red' : 'orange';
      
      tr.innerHTML = `
        <td>${esc(patient.name)}</td>
        <td>${esc(patient.email)}</td>
        <td><span class="badge" style="background: ${statusColor}20; color: ${statusColor};">${esc(patient.status)}</span></td>
  ...
  function renderDoctorsTable(query) {
    ...
    if (filtered.length === 0) {
      doctorsTableBody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 20px; color: #999;">No doctors found</td></tr>';
      return;
    }
    ...
    filtered.forEach((doctor) => {
      ...
      const statusColor = doctor.status === 'approved' ? 'green' : doctor.status === 'rejected' ? 'red' : 'orange';
      
      tr.innerHTML = `
        <td>${esc(doctor.name)}</td>
        <td>${esc(doctor.email)}</td>
        <td>${esc(doctor.speciality || "—")}</td>
        <td><span class="badge" style="background: ${statusColor}20; color: ${statusColor};">${esc(doctor.status)}</span></td>
        <td>${date}</td>
        <td>
  ...
  async function loadLoginHistory() {
    try {
      ...
      if (loginHistoryTableBody) {
        loginHistoryTableBody.innerHTML = `<tr><td colspan="7" style="color: red; text-align: center;">Error loading login history: ${esc(e.message)}</td></tr>`;
      }
    }
  }
  ...
  function renderLoginHistoryTable(query) {
    ...
    if (filtered.length === 0) {
      loginHistoryTableBody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px; color: #999;">No login records found</td></tr>';
      return;
    }
    ...
    filtered.forEach((record) => {
      ...
      const formattedDate = loginDate.toLocaleDateString() + " " + loginDate.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
      ...
      tr.innerHTML = `
        ...
        <td>${formattedDate}</td>
        <td><code style="background: #f0f0f0; padding: 4px 8px; border-radius: 3px; font-size: 0.9em;">${esc(record.ipAddress || '—')}</code></td>
